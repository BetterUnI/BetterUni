...
import "./style.scss";
import redDoubleTick from "./resources/red-double-tick-icon.svg";
...
class SenderMessageBubble extends React.Component {
  ...
  displayTicks = message => {
    if (message.sentAt && !message.readAt && !message.deliveredAt) {
      ...
      return redDoubleTick;
    }
  };
  render() {
    return (
      <div className=" cp-sender-message-container">
        <div className=" cp-sender-message">{this.state.message.text}</div>
        <div className="cp-time text-muted">
          {" "}
          {new Date(this.state.message.sentAt * 1000).toLocaleTimeString(
            "en-US",
            {
              hour: "numeric",
              minute: "numeric",
          ...
          <span>
            <img src={this.displayTicks(this.state.message)} alt="time"></img>
          </span>
        </div>
      </div>
    );
